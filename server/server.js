...
async function getStartManifest() {
  const { tsrStartManifest } = await import("./assets/_tanstack-start-manifest_v-GX9sx_sS.js");
...
const handleServerAction = async ({
  ...
  if (regex === void 0) {
    regex = new RegExp(`${"/naofix_widget_poc/_serverFn/"}([^/?#]+)`);
  }
...
function createStartHandler(cb) {
  const ROUTER_BASEPATH = "naofix_widget_poc";
  let startRoutesManifest = null;
  let startEntry = null;
  let routerEntry = null;
  const getEntries = async () => {
    if (routerEntry === null) {
      routerEntry = await import("./assets/router-BBsnP-9i.js").then((n) => n.j);
    }
    if (startEntry === null) {
      ...
      const requestHandlerMiddleware = handlerToMiddleware(
        async ({ context }) => {
          const response2 = await runWithStartContext(
            ...
            async () => {
              try {
                if (href.startsWith("/naofix_widget_poc/_serverFn/")) {
